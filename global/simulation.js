    ...
    for (let x = 0; x < 80; x++) {
        for (let y = 0; y < 48; y++) {
            if ((wrld[x][y].flwr == 'b' || wrld[x][y].flwr == 'w') && wrld[x][y].flwr.includes('t') !== true) {
                ...
                if (Math.random() < 1.4 * (1 - 0.000103 * Math.pow(100 - localPop, 2))) {
                    wrld[x][y].flwr = undefined;
                    wrld[x][y].genetics = 1;
                    gPop[wrld[x][y].flwr]--;

                } else if (Math.random() < 1 - 0.0034 * Math.pow(pT[pT.length - 1] / 15, 2)) {
...
function draw() {
    //steps++
    //sum += pT[pT.length - 1]
    //console.log(pT[pT.length - 1])
    //console.log("average = ", sum/steps)
    ...
    for (let x = 0; x < 80; x++) {
        for (let y = 0; y < 48; y++) {
            ...
            if (wrld[x][y].temp > 0) {
                ctx.globalAlpha = wrld[x][y].temp/2;
                ctx.fillRect(x * sX, y * sY, sX + mg, sY + mg);
            }
